...
if __name__ == "__main__":
    ...
    logger.info("Iniciando instância da sub-aplicação Conversor XMLs")
    try:
        app = AppConversorXML()
        app.mainloop()
        logger.info("Sub-aplicação Conversor XMLs encerrada normalmente")
    except Exception:
        logger.exception("Falha crítica na execução da sub-aplicação Conversor XMLs")
        sys.exit(1)
