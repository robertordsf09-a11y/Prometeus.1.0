"""
Excel Password Protector v3 - Final com Logs
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
• LOG DETALHADO COMPLETO (Relatório, Resumo e Verificação) conforme original
...
def configurar_log(caminho_log: str):
    """Configura o logger para escrever no arquivo de log conforme o original."""
    ...
    _logger.info("=" * 70)
    _logger.info("Sessao iniciada — Excel Password Protector v3")
    _logger.info("=" * 70)
...
class App(ctk.CTk):
    ...
    def _setup_ui(self):
        ...
        # Footer
        footer = ctk.CTkFrame(self, fg_color=BG_CARD, height=120, corner_radius=0)
        ...
        self._lbl_log_path = ctk.CTkLabel(footer, text="", text_color=SUBTEXTO, font=ctk.CTkFont(size=10))
        self._lbl_log_path.pack(side="left", padx=5)
    ...
    def _iniciar(self):
        ...
        if not selecionados or len(senha) < 4 or not destino:
            self._lbl_status.configure(text="⚠️ Verifique os campos!", text_color=AMARELO)
            return
    ...
    def _processar(self, origem_path, destino_path, senha, selecionados):
        ...
        self.after(0, lambda: self._lbl_log_path.configure(text=f"📋 Log: {log_path}"))
        ...
        _logger.info("Origem  : %s", origem_path)
        _logger.info("Destino : %s", dest_final)
        _logger.info("Total de arquivos : %d", total)
        _logger.info("Marcados para protecao : %d", len(set_selecionados))
        _logger.info("-" * 70)
        ...
        for i, arq_abs in enumerate(todos, 1):
            ...
            if rel in set_selecionados:
                ...
                res = proteger_excel(arq_abs, dest_arq, senha)
                if res.sucesso:
                    cnt_prot_ok += 1
                    _logger.info("  OK  Criptografado e verificado (metodo: %s)", res.metodo)
                else:
                    cnt_prot_err += 1
                    _logger.error("  ERRO  Falha ao proteger: %s", res.erro)
            else:
                ...
                try:
                    ...
                    cnt_erros_cp += 1
                    _logger.error("  ERRO  Falha ao copiar: %s", exc)
