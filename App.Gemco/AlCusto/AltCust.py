...
if __name__ == "__main__":
    ...
    logger.info("Iniciando instância da sub-aplicação AlCusto")
    try:
        app = AutomacaoApp()
        app.mainloop()
        logger.info("Sub-aplicação AlCusto encerrada normalmente")
    except Exception:
        logger.exception("Falha crítica na execução da sub-aplicação AlCusto")
        sys.exit(1)